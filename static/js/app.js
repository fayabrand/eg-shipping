...
document.addEventListener('DOMContentLoaded', () => {
  const d = document.getElementById('cl-date');
  if (d) d.value = new Date().toISOString().split('T')[0];
  loadState().then(() => { renderBoxes(); updateNavStats(); fillExportSelects(); });
});
...
async function inlineScanItem() {
  ...
  const bc = document.getElementById('inline-barcode');
  const barcode = bc.value.trim();
  if (!barcode) return;
  ...
  if (res.status === 409) {
    ...
    fb.className = 'feedback err'; fb.textContent = 'Duplicate: ' + err.error; fb.style.display = 'block';
    bc.select(); return;
  }
  await loadState();
  ...
  bc.value = ''; document.getElementById('inline-desc').value = '';
  bc.focus(); renderInlineItems(); updateNavStats();
...
function renderInlineItems() {
  ...
  list.innerHTML = [...box.items].reverse().map(it => `
    <div class="item-row">
      ...
      <div class="item-meta">${it.description||''} ${it.description?'·':''} ${cap(it.category)} · ${it.time||''}</div>
...
async function inlineDeleteItem(barcode) {
  ...
  await loadState();
  const box = state.boxes.find(b => b.id === activeScanBoxId);
  if (box) document.getElementById('inline-box-label').textContent = box.id + ' — ' + box.client + ' (' + box.items.length + ' items)';
  renderInlineItems(); updateNavStats();
}
...
async function inlineCloseBox() {
  ...
  await loadState(); renderBoxes(); updateNavStats(); closeInlineScan();
}
  ...
  el.innerHTML = boxes.map(b => `
    <div class="box-card ${b.status}">
      ...
      <div class="box-actions">
        ...
        ${b.status==='open'?'<button class="btn sm primary" onclick="openInlineScan(''+b.id+'')">📷 Add Items</button>':''}
...
async function scanItem() {
  ...
  fb.className = 'feedback ok'; fb.textContent = 'Added: ' + barcode; fb.style.display = 'block';
  bc.value = ''; if (document.getElementById('item-desc')) document.getElementById('item-desc').value = '';
...
function renderScannedItems() {
  ...
  list.innerHTML = [...box.items].reverse().map(it =>
    '<div class="item-row"><div class="item-bc">'+it.barcode+'</div><div class="item-meta">'+(it.description||'')+' · '+cap(it.category)+' · '+(it.time||'')+'</div><button class="btn sm danger" onclick="deleteItem(''+box.id+'',''+it.barcode+'')">✕</button></div>'
  ).join('');
}
...
async function doLookup() {
  ...
  if (data.found) {
    el.innerHTML = '<div class="result-card result-found"><div class="result-header result-found"><div class="result-icon">✅</div><div><div class="result-title">'+barcode+'</div><div class="result-sub">Found</div></div></div><div style="padding:14px 18px"><div><strong>Box:</strong> '+data.box.id+' — '+data.box.client+'</div><div><strong>Date:</strong> '+data.box.date+'</div><div><strong>Status:</strong> '+data.box.status+'</div><div><strong>Category:</strong> '+cap(data.item.category)+'</div>'+(data.item.description?'<div><strong>Desc:</strong> '+data.item.description+'</div>':'')+'</div></div>';
  } else {
...
function liveSearch() {
  ...
  if (!v) { const el = document.getElementById('lookup-result'); if (el) el.innerHTML = ''; }
}

function clearLookup() {
  const i = document.getElementById('lookup-input'); if (i) i.value = '';
  const r = document.getElementById('lookup-result'); if (r) r.innerHTML = '';
}
...
function updateNavStats() {
  ...
  const open  = state.boxes.filter(b => b.status === 'open').length;
  const items = state.boxes.reduce((a, b) => a + b.items.length, 0);
  el.innerHTML = '<span>'+total+' boxes</span><span>'+items+' items</span><span>'+open+' open</span>';
}
...
function printBoxById(id) {
  ...
  w.document.write('<html><head><title>Receipt '+box.id+'</title><style>body{font-family:Arial,sans-serif;padding:20px;max-width:600px}h2{margin-bottom:4px}.meta{color:#666;font-size:13px;margin-bottom:16px}table{width:100%;border-collapse:collapse}th,td{padding:8px;text-align:left;border-bottom:1px solid #eee}th{background:#f5f5f5;font-size:12px;color:#666}.footer{margin-top:20px;font-size:12px;color:#999;border-top:1px solid #eee;padding-top:10px}</style></head><body><h2>'+box.id+' — '+box.client+'</h2><div class="meta">Phone: '+(box.phone||'—')+' | Date: '+(box.date||'—')+' | Origin: '+(box.origin||'').toUpperCase()+' | '+cap(box.category)+' | Status: '+box.status+'</div><table><thead><tr><th>#</th><th>Barcode</th><th>Description</th><th>Category</th><th>Time</th></tr></thead><tbody>'+box.items.map((it,i)=>'<tr><td>'+(i+1)+'</td><td>'+it.barcode+'</td><td>'+(it.description||'—')+'</td><td>'+cap(it.category)+'</td><td>'+(it.time||'—')+'</td></tr>').join('')+'</tbody></table><div class="footer">Total items: '+box.items.length+' | EG Shipping Receiver</div></body></html>');
...
async function importData(input) {
  ...
  if (d.ok) {
    await loadState(); renderBoxes(); updateNavStats();
    const fb = document.getElementById('backup-feedback');
    fb.className = 'feedback ok'; fb.textContent = 'Imported ' + d.boxes + ' boxes'; fb.style.display = 'block';
    setTimeout(() => { fb.style.display = 'none'; }, 3000);
...
async function renderReports() {
  ...
  const total   = state.boxes.length;
  const open    = state.boxes.filter(b => b.status==='open').length;
  const closed  = state.boxes.filter(b => b.status==='closed').length;
  const items   = state.boxes.reduce((a,b) => a+b.items.length, 0);
  const clients = new Set(state.boxes.map(b => b.client)).size;
  const mr = document.getElementById('metrics-row');
  if (mr) mr.innerHTML = '<div class="metric"><div class="metric-val">'+total+'</div><div class="metric-lbl">Boxes</div></div><div class="metric"><div class="metric-val">'+open+'</div><div class="metric-lbl">Open</div></div><div class="metric"><div class="metric-val">'+closed+'</div><div class="metric-lbl">Closed</div></div><div class="metric"><div class="metric-val">'+items+'</div><div class="metric-lbl">Items</div></div><div class="metric"><div class="metric-val">'+clients+'</div><div class="metric-lbl">Clients</div></div>';
  const rl = document.getElementById('report-list');
  if (!rl) return;
  if (!state.boxes.length) { rl.innerHTML = '<div class="empty">No data yet</div>'; return; }
  rl.innerHTML = '<table class="data-table"><thead><tr><th>Box ID</th><th>Client</th><th>Date</th><th>Category</th><th>Origin</th><th style="text-align:center">Items</th><th style="text-align:center">Status</th></tr></thead><tbody>' +
    state.boxes.map(b=>'<tr><td style="font-family:monospace;font-size:12px">'+b.id+'</td><td><b>'+b.client+'</b><br><span class="muted" style="font-size:11px">'+(b.phone||'')+'</span></td><td class="muted" style="font-size:12px">'+(b.date||'')+'</td><td><span class="badge '+b.category+'">'+cap(b.category)+'</span></td><td><span class="badge '+oc(b.origin)+'">'+((b.origin||'').toUpperCase())+'</span></td><td style="text-align:center;font-weight:600">'+b.items.length+'</td><td style="text-align:center"><span class="badge '+b.status+'">'+b.status+'</span></td></tr>').join('') +
    '</tbody></table>';
}
